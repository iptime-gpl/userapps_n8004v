...
#define RESET_BT_GPIO 2  /* A2 */
#define RESET_BT_GPIO_BIT (0x1<<RESET_BT_GPIO)

#define RED_LED_GPIO (8+6)   /* B6 : RED */
#define RED_LED_GPIO_BIT (0x1<<RED_LED_GPIO) /* PAC */

#define BLUE_LED_GPIO  (6)  /* A6 : BLUE */
#define BLUE_LED_GPIO_BIT (0x1<<BLUE_LED_GPIO)

#define EXT_2G_WPS_BT  	(5)  /* A5 : Same as Reset */ 
...
#define LED_OP_STATIC 0
#define LED_OP_BLUE_BLINKING 1 
#define LED_OP_RED_BLINKING 2
#define LED_OP_BLUE_FAST_BLINKING 3
#define LED_OP_RED_FAST_BLINKING 4
#define LED_OP_PURPLE_FAST_BLINKING 5

int ext_dual_led_op = LED_OP_STATIC;
...
int ext_led_operation(int op, int count)
{
	switch(op)
	{
		case LED_OP_STATIC:
			break;
		case LED_OP_BLUE_FAST_BLINKING:
                	blink_led(BLUE_LED_GPIO,-1);
			break;
		case LED_OP_RED_FAST_BLINKING:
                	blink_led(RED_LED_GPIO,-1);
			break;
		case LED_OP_PURPLE_FAST_BLINKING:
                	blink_led(RED_LED_GPIO,BLUE_LED_GPIO);
			break;
		case LED_OP_BLUE_BLINKING:
			if(!(count%5)) 
			{
				blink_led(BLUE_LED_GPIO,-1);
...
{
	led_run_count++;
	ext_led_operation(ext_dual_led_op,led_run_count);

	if(reset_start)
	{
		ext_dual_led_op=LED_OP_RED_FAST_BLINKING;
		set_led_op(BLUE_LED_GPIO,0);
	}
	else 
	if(boot_completed)
	{
		if(prev_ext2g_bt_status && !check_ext2g_wps_bt())
		{
			...
			ext_dual_led_op = LED_OP_BLUE_FAST_BLINKING;
			set_led_op(RED_LED_GPIO,0);
...
static int proc_write_boot_completed( struct file *file, const char *buffer, u_long count, void *data )
{
	...
	{
		boot_completed=1;
		ext_dual_led_op=LED_OP_RED_BLINKING;
	}
        return count;
}
...
static int proc_write_ext_dual_led( struct file *file, const char *buffer, u_long count, void *data )
{
	if(!strncmp(buffer,"on_red",6))
	{
		...
		set_led_op(BLUE_LED_GPIO,0);
		ext_dual_led_op = LED_OP_STATIC;
	}
	else if(!strncmp(buffer,"on_blue",7))
	{
		set_led_op(RED_LED_GPIO,0);
		set_led_op(BLUE_LED_GPIO,1);
		ext_dual_led_op = LED_OP_STATIC;
	}
	else if(!strncmp(buffer,"on_purple",9))
	{
		set_led_op(RED_LED_GPIO,1);
		set_led_op(BLUE_LED_GPIO,1);
		ext_dual_led_op = LED_OP_STATIC;
	}
	else if(!strncmp(buffer,"blink_blue",10))
	{
		ext_dual_led_op = LED_OP_BLUE_BLINKING;
		set_led_op(RED_LED_GPIO,0);
	}
	else if(!strncmp(buffer,"blink_red",9))
	{
		ext_dual_led_op = LED_OP_RED_BLINKING;
		set_led_op(BLUE_LED_GPIO,0);
	}
	else if(!strncmp(buffer,"fblink_blue",11))
	{
		ext_dual_led_op = LED_OP_BLUE_FAST_BLINKING;
		set_led_op(RED_LED_GPIO,0);
	}
	else if(!strncmp(buffer,"fblink_red",10))
	{
		ext_dual_led_op = LED_OP_RED_FAST_BLINKING;
		set_led_op(BLUE_LED_GPIO,0);
	}
	...
	else if(!strncmp(buffer,"fblink_purple",13))
	{
		ext_dual_led_op = LED_OP_PURPLE_FAST_BLINKING;
		set_led_op(RED_LED_GPIO,0);
		set_led_op(BLUE_LED_GPIO,0);
	}
...
static  int init_gpio_mode(void)
{
	RTL_W32(PIN_MUX_SEL_2, RTL_R32(PIN_MUX_SEL_2)|0x600|0x3000);
        ...
        RTL_W32(PABCD_CNR, (RTL_R32(PABCD_CNR) & (~RED_LED_GPIO_BIT) & (~RESET_BT_GPIO_BIT) & (~BLUE_LED_GPIO_BIT) & (~EXT_2G_WPS_BT_GPIO_BIT)));
        ...
        RTL_W32(PABCD_DIR, (RTL_R32(PABCD_DIR) | BLUE_LED_GPIO_BIT));
...
static irqreturn_t wps_gpio_interrupt_isr(int irq, void *dev_instance, struct pt_regs *regs)
{
        ...
        if(RTL_R32(PABCD_ISR) & WPS_BT_GPIO_BIT)
        {
                ...
                {
                        printk("Send WPS signal.. \n");
			//ext_dual_led_op = LED_OP_WPS_CONNECTING;
                        send_sig_pid(SIGPWR, 1); /* send WPS signal */
                }
        }
