...
#define RESET_BT_GPIO (8+5)  /* B5 */
#define RESET_BT_GPIO_BIT (0x1<<RESET_BT_GPIO)

#define RED_LED_GPIO 4   /* A4 : RED */
#define RED_LED_GPIO_BIT (0x1<<RED_LED_GPIO) /* PAC */

#define GREEN_LED_GPIO  (8+6)  /* B6 : GREEN */
#define GREEN_LED_GPIO_BIT (0x1<<GREEN_LED_GPIO)

#define EXT_2G_WPS_BT  	(8+5)  /* B5 : Same as Reset */ 
#define EXT_2G_WPS_BT_GPIO_BIT (0x1<<EXT_2G_WPS_BT)

#define EXT_INDICATION_GPIO 	(8+6)  /* B6 : GREEN */
#define EXT_INDICATION_GPIO_BIT  (0x1<<EXT_INDICATION_GPIO)  /* A2 */


int run_period = HZ;
#define LED_OP_FAST_BLINK 0x5
int reset_op_start;
...
static int check_ext2g_wps_bt(void)
{
        ...
        if(gpio_value & EXT_2G_WPS_BT_GPIO_BIT)
                return 0; /* not pushed */
        else
                return 1; /* pushed */
}
...
//      printk("-> GPIO_DATA: %08x\n", gpio_shadow);
        if( gpio_shadow & RED_LED_GPIO_BIT)
                gpio_shadow &= ~RED_LED_GPIO_BIT;
        else
                gpio_shadow |= RED_LED_GPIO_BIT;
...
int blink_led(int gpio, int gpio2)
{
        gpio_shadow = RTL_R32(PABCD_DAT);
        if(gpio_shadow & (0x1<<gpio))
	{
                gpio_shadow &= ~(0x1 <<gpio);
		if(gpio2!=-1) gpio_shadow &= ~(0x1 <<gpio2);
	}
        else
	{
                gpio_shadow |= (0x1 <<gpio);
...
{
        gpio_shadow = RTL_R32(PABCD_DAT);
        gpio_shadow &= ~EXT_INDICATION_GPIO_BIT;
        RTL_W32(PABCD_DAT, gpio_shadow);
        return 0;
}
...
#define LED_OP_INIT_STATUS    0x0  /* Red ON */
#define LED_OP_BOOT_COMPLETED 0x1  /* Yellow ON */
#define LED_OP_WPS_CONNECTING 0x2  /* Yellow Fast Blinking */
#define LED_OP_WIRELESS_CONNECTING 0x3 /* Green Slow Blinking */
#define LED_OP_WIRELESS_CONNECTED 0x4  /* Green ON */
#define LED_OP_FACTORY_DEFAULT 0x5     /* Red Fast Blinking */
#define LED_OP_FORCED_OFF 0x6  
#define LED_OP_TEST 0x7 

int ext_2g_led_op = LED_OP_INIT_STATUS;

int ext_led_operation(int op, int count)
{
	switch(op)
	{
		case LED_OP_INIT_STATUS:
			set_led_op(RED_LED_GPIO,1);
			set_led_op(GREEN_LED_GPIO,0);
			break;
		case LED_OP_BOOT_COMPLETED:
			set_led_op(RED_LED_GPIO,1);
			set_led_op(GREEN_LED_GPIO,1);
			break;
		case LED_OP_WPS_CONNECTING:
                	blink_led(RED_LED_GPIO,GREEN_LED_GPIO);
			break;
		case LED_OP_WIRELESS_CONNECTING:
			if(!(count%5)) 
			{
				blink_led(GREEN_LED_GPIO,-1);
				set_led_op(RED_LED_GPIO,0);
			}
			break;
		case LED_OP_WIRELESS_CONNECTED:
			set_led_op(RED_LED_GPIO,0);
			set_led_op(GREEN_LED_GPIO,1);
			break;
		case LED_OP_FACTORY_DEFAULT:
                	blink_led(RED_LED_GPIO,-1);
			set_led_op(GREEN_LED_GPIO,0);
			break;
		case LED_OP_FORCED_OFF:
			set_led_op(RED_LED_GPIO,0);
			set_led_op(GREEN_LED_GPIO,0);
			break;
		case LED_OP_TEST:
...
#ifndef USE_IRQ_FOR_WPS
#ifdef CONFIG_EFM_PATCH
int led_run_count;

int prev_ext2g_bt_status;

static void rtl_wps_timer(unsigned long data)
{
	led_run_count++;
	ext_led_operation(ext_2g_led_op,led_run_count);

	if(reset_start)
		ext_2g_led_op=LED_OP_FACTORY_DEFAULT;
	else if(boot_completed)
	{
		if(prev_ext2g_bt_status && !check_ext2g_wps_bt())
		{
                        printk("Send WPS signal..\n");
			ext_2g_led_op = LED_OP_WPS_CONNECTING;
                        send_sig_pid(SIGPWR, 1); /* send WPS signal */
                }
		prev_ext2g_bt_status = check_ext2g_wps_bt();
	}

	if(!boot_completed)
        	mod_timer(&wps_timer, jiffies + 8*HZ); /* This delay is important */
	else
        	mod_timer(&wps_timer, jiffies + HZ/10);
}

static int proc_write_boot_completed( struct file *file, const char *buffer, u_long count, void *data )
{
        if(!strncmp(buffer,"1",1))
	{
		boot_completed=1;
		ext_2g_led_op=LED_OP_BOOT_COMPLETED;
	}
        return count;
}
...
__NOMIPS16
#endif
static int proc_read_reset_start(char *buffer, char **start, off_t offset, int length, int *eof, void *data)
{
        char *p = buffer;
        int len;

        p +=  sprintf(p, "%d", reset_start);
...
{
	if(!strncmp(buffer,"on_red",6))
	{
		set_led_op(RED_LED_GPIO,1);
	}
	else if(!strncmp(buffer,"off_red",7))
	{
		set_led_op(RED_LED_GPIO,0);
	}
	else if(!strncmp(buffer,"on_green",8))
	{
		set_led_op(GREEN_LED_GPIO,1);
	}
	else if(!strncmp(buffer,"off_green",9))
	{
		set_led_op(GREEN_LED_GPIO,0);
	}
        else if(!strncmp(buffer,"on",2))
	{
		ext_2g_led_op = LED_OP_WIRELESS_CONNECTED;
	}
	else if(!strncmp(buffer,"off",3))
	{
		ext_2g_led_op = LED_OP_FORCED_OFF;
	}
	...
	{
		ext_2g_led_op = LED_OP_WIRELESS_CONNECTING;
	}
	else if(!strncmp(buffer,"noconfig",8))
	{
		ext_2g_led_op = LED_OP_BOOT_COMPLETED;
	}
	else if(!strncmp(buffer,"test",4))
	{
		ext_2g_led_op = LED_OP_TEST;
	}
...
{
	RTL_W32(PIN_MUX_SEL_2, RTL_R32(PIN_MUX_SEL_2)|0x600|0x3000);

        RTL_W32(PABCD_CNR, (RTL_R32(PABCD_CNR) & (~RED_LED_GPIO_BIT) & (~RESET_BT_GPIO_BIT) & (~EXT_INDICATION_GPIO_BIT) & (~GREEN_LED_GPIO_BIT) & (~EXT_2G_WPS_BT_GPIO_BIT)));

        /* Set GPIO Direction */
        RTL_W32(PABCD_DIR, (RTL_R32(PABCD_DIR) & (~RESET_BT_GPIO_BIT)));
        RTL_W32(PABCD_DIR, (RTL_R32(PABCD_DIR) & (~EXT_2G_WPS_BT_GPIO_BIT)));
        RTL_W32(PABCD_DIR, (RTL_R32(PABCD_DIR) | RED_LED_GPIO_BIT));
        RTL_W32(PABCD_DIR, (RTL_R32(PABCD_DIR) | GREEN_LED_GPIO_BIT));
...
static  int init_iptime_led_spec(void)
{
        ...
        sprintf(name, "driver/reset_start");
        proc_entry = create_proc_entry(name,0,0);
        if(proc_entry)
                proc_entry->read_proc=&proc_read_reset_start;
        ...
        if(proc_entry)
	{
        ...
        sprintf(name, "driver/boot_completed");
        proc_entry = create_proc_entry(name,0,0);
        if(proc_entry)
                proc_entry->write_proc=&proc_write_boot_completed;
