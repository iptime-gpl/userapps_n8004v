...
//	rtl865x_wanPortMask = RTL865X_PORTMASK_UNASIGNED;
	...
	extern int32 smi_init(uint32 port, uint32 pinSCK, uint32 pinSDA);
        ...
        extern int RTL8367R_VB_init(void);
#else
	extern int RTL8370_init(void);
#endif
...
/*	WRITE_MEM32(MACCR,READ_MEM32(MACCR)&~DIS_IPG);//Set IFG range as 96+-4bit time*/
	...
	rtl8651AsicEthernetTable[5].phyId = P0_EXT_PHY_ID;
	...
	*/
	...
	else //external phy
	{
		...
		P0miimode = 3;
                ...
                //REG32(PCRP0) |=  (0x06 << ExtPHYID_OFFSET) | MIIcfg_RXER |  EnablePHYIf | MacSwReset;   //external - original
		...
		if(P0miimode==3)
		{
			...
			P0rxdly=Get_P0_RxDelay();
                        ...
                        P0txdly = 1;
                        P0rxdly = 7;
                        REG32(0xb8000048) |= (1<<28); // PASS_PIN_RGMII_MODE=1
			#elif defined(CONFIG_RTL_8367R_SUPPORT)
			P0txdly = 1;
			#ifdef CONFIG_RTL_8367R_BOARD_04281A_SUPPORT
			P0rxdly = 7;
			#else
			P0rxdly = 5;
			#endif
...
int32 rtl8651_setAsicEthernetPHYSpeed( uint32 port, uint32 speed )
{
	...
	printk("rtl8651_setAsicEthernetPHYSpeed(), port %d \n", port);
	...
	printk("                         phyid %d,  speed %d \n",  rtl8651AsicEthernetTable[port].phyId,  speed);
