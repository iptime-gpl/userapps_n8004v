 ...
 * All rights reserved.
 *
 * $Header: /usr/kcvsrep/RTL3.4/rtl819x/linux-2.6.30/drivers/net/rtl819x/rtl_nic.c,v 1.25 2017/08/03 08:45:14 rtlac Exp $
 *
 * $Author: rtlac $
 *
...
int  rtl_MulticastRxCheck(struct sk_buff *skb,rtl_nicRx_info *info)
{
	...
	if((skb->data[0]==0x01) && (skb->data[1]==0x00)&& (skb->data[2]==0x5e))
	{
		...
		if(iph!=NULL)
		{
			...
			/*
			if (memcmp(&skb->data[0], SSDP_mc_addr, 6) == 0)
				printk("Rx SSDP 2 from %d (%08x)\n", pid, vlanRelayPortMask);
			*/	
			...
			else if(l4Protocol==IPPROTO_IGMP)
			{
				...
				if(igmpsnoopenabled && (nicIgmpModuleIndex!=0xFFFFFFFF))
				{
...
#ifdef CONFIG_RTL8367R_VB_API
                    {
                    char  *igmph = (u8*)iph + (iph->ihl * 4);
                    rtk_mac_t SrcMac, GroupMac;
                    rtk_l2_ucastAddr_t L2_data;

                    memcpy(SrcMac.octet, &skb->data[6], 6);
                    L2_data.fid = 0;

                    #if 0
                    printk("igmp mac [%d] : %02x-%02x-%02x-%02x-%02x-%02x\n", *igmph,
                            SrcMac.octet[0], SrcMac.octet[1], SrcMac.octet[2], SrcMac.octet[3], SrcMac.octet[4], SrcMac.octet[5]);
                    #endif
                    ...
                    if (rtk_l2_addr_get(&SrcMac, &L2_data) == RT_ERR_OK)
                    {
                            ...
                            //printk("--> L2data(%d) port : %d \n", L2_data.fid, L2_data.port);

                            GroupMac.octet[0] = 0x01; GroupMac.octet[1] = 0x0; GroupMac.octet[2] = 0x5e;
                            memcpy(&GroupMac.octet[3], igmph+5, 3);
                            GroupMac.octet[3] &= 0x7f;

                            #if 0
                            printk("--> Gmac : %02x-%02x-%02x-%02x-%02x-%02x    ",
                                    GroupMac.octet[0], GroupMac.octet[1], GroupMac.octet[2],
                                    GroupMac.octet[3], GroupMac.octet[4], GroupMac.octet[5]);
                            #endif

                            portmask.bits[0] = 0;
                            rtk_l2_mcastAddr_get(&GroupMac, 0, &portmask);

                            if (*igmph == 0x16 || *igmph == 0x12) // Join
                            {
                                    portmask.bits[0] |= (1 << 9); // GMAC0
                                    portmask.bits[0] |= (1 << L2_data.port);
                                    rtk_l2_mcastAddr_add(&GroupMac, 0, portmask);
                            }
                            else if (*igmph == 0x17) // Leave
                            {
                                    portmask.bits[0] &= ~(1 << L2_data.port);
                                    rtk_l2_mcastAddr_add(&GroupMac, 0, portmask);

                                    if (portmask.bits[0] == (1 << 9))
                                            rtk_l2_mcastAddr_del(&GroupMac, 0);
                            }
                            #if 0
                            portmask.bits[0] = 0;
                            rtk_l2_mcastAddr_get(&GroupMac, 0, &portmask);
                ...
                else if (iptv_port != -1)
                {
                        ...
                        printk("IPTV IGMP packet GET \n");

                        portmask.bits[0] = 0;
						vlanRelayPortMask = rtl865x_getVlanPortMask(vid) & ((1<<RTL8651_MAC_NUMBER)-1);

                        GroupMac.octet[0] = 0x01; GroupMac.octet[1] = 0x0; GroupMac.octet[2] = 0x5e;
                        memcpy(&GroupMac.octet[3], igmph+5, 3);
                        GroupMac.octet[3] &= 0x7f;

						if ((memcmp(&GroupMac.octet[0], &SSDP_mc_addr[0], 6) == 0) ||
			        		(memcmp(&GroupMac.octet[0], &iTunes_mc_addr[0], 6) == 0))
						{
								printk("KT IPTV: IGMP Filtering for SSDP & MDNS\n");
								return 0;
						}

                        rtk_l2_mcastAddr_get(&GroupMac, 0, 0, &portmask);

                        if (*igmph == 0x16 || *igmph == 0x12) // Join
                        {
                        	printk("      JOIN  \n");
                                portmask.bits[0] = (1 << 0); // WAN
                                portmask.bits[0] |= (1 << iptv_port);
                                rtk_l2_mcastAddr_add(&GroupMac, 0, 0, portmask);
                                vlanRelayPortMask &= ~(1 << iptv_port);
                        }
                        else if (*igmph == 0x17) // Leave
                        {
                        	printk("      LEAVE  \n");
                                rtk_l2_mcastAddr_del(&GroupMac, 0, 0);
								vlanRelayPortMask &= ~(1 << iptv_port);
                        }
                        else if (*igmph == 0x11) // Query
								vlanRelayPortMask &= ~(1 << 0); // WAN

                       	printk("--> Gmac : %02x-%02x-%02x-%02x-%02x-%02x   (%08x, vid %d) vlanmask:%08x \n ",
                               	GroupMac.octet[0], GroupMac.octet[1], GroupMac.octet[2],
                                GroupMac.octet[3], GroupMac.octet[4], GroupMac.octet[5], 
								portmask.bits[0], vid, vlanRelayPortMask);
                } 
...
static inline int32 rtl_decideRxDevice(rtl_nicRx_info *info)
{
	...
	#endif
	{
		...
		for(i = 0; i < ETH_INTF_NUM; i++)
		{
			...
			if(cp && cp->opened && (cp->id == info->vid))
			{
				info->priv = cp;
				ret = SUCCESS;
				break;
...
static struct proc_dir_entry *iptv_port_entry;
static struct proc_dir_entry *wan_port_type_entry;
...
static int32 proc_write_iptv_port( struct file *filp, const char *buff,unsigned long len, void *data )
{
	...
	/* New add during N804T3/N904 porting */ 
	/* iptv_port was in the LAN VLAN originally. So remove this from the LAN VLAN */
       	rtl865x_delVlanPortMember(RTL_LANVLANID & 4095, (0x1<<iptv_port));
	/* New add end: scchoi */

        rtl865x_addVlanPortMember(RTL_WANVLANID & 4095, (0x1<<iptv_port));
        rtl8651_setAsicPvid(iptv_port,     RTL_WANVLANID);
        ((struct dev_priv *)_rtl86xx_dev.dev[0]->priv)->portmask = rtl865x_getVlanPortMask(RTL_LANVLANID);
        ((struct dev_priv *)_rtl86xx_dev.dev[1]->priv)->portmask = rtl865x_getVlanPortMask(RTL_WANVLANID);

	/* rtl_setIpv4UnknownMCastFloodMap(nicIgmpModuleIndex, (0x1 | (0x1<<iptv_port))); */
#ifdef CONFIG_RTL_8367R_SUPPORT
	//rtk_trap_igmpCtrlPktAction_set( IGMP_IPV4, IGMP_ACTION_FORWARD); /* RTL8367R config */
#endif
	...
	rtk_trap_unknownMcastPktAction_set(RTL8370_WAN, IGMP_IPV4, IGMP_ACTION_FORWARD);
        ...
        printk("LAN portmask : %08x WAN portmask: %08x\n" , ((struct dev_priv *)_rtl86xx_dev.dev[0]->priv)->portmask , ((struct dev_priv *)_rtl86xx_dev.dev[1]->priv)->portmask );
...
#ifdef CONFIG_RTL_8367R_SUPPORT
	{
	unsigned int PriDec[8] = {0x01,0x40,0x80,0x02,0x10,0x20,0x08,0x04}; // DSCP:1
	unsigned int Pri2qid[8] = {0, 0,1,1,2,3,7,7};
	unsigned int Qweights[8] = {0,0,0,0,0,0,0,0}; // Strict priority
	int dscp, port;

	rtk_qos_init(4);
	rtk_qos_priSel_set(&PriDec);
	rtk_qos_priMap_set(4, &Pri2qid);
	//rtk_qos_dscpRemarkEnable_set(0xff, 1); // RTK_WHOLE_SYSTEM :0xff
...
#endif
                {
/* printk("RTL8367R PORT STATUS GET:%d\n", portnum); */
			...
			//printk("RTL8367R PORT STATUS GET:%d - link %d, speed %d, duplex %d\n", portnum, link, speed, duplex); 
			#if 0
			if (portnum == 7)
			{
				portnum = 8;
                        	rtk_port_phyStatus_get(portnum, &link, &speed, &duplex);
				printk("RTL8370 PORT STATUS GET:8 - link %d, speed %d, duplex %d\n",  link, speed, duplex); 
				portnum = 9;
                        	rtk_port_phyStatus_get(portnum, &link, &speed, &duplex);
				printk("RTL8370 PORT STATUS GET:9 - link %d, speed %d, duplex %d\n",  link, speed, duplex); 
			}
			#endif
                }
#ifdef CONFIG_RTL_8370_SUPPORT
		else if(portnum == 8) // WAN port
		{
			rtl8651_getAsicEthernetPHYReg(P0_EXT_PHY_ID, 1, &regData);
			rtl8651_getAsicEthernetPHYReg(P0_EXT_PHY_ID, 1, &regData);
			//printk("P0_EXT_PHY_ID : reg 1, value %x \n", regData);
			...
			if (port_status->link)
			{
				...
				//printk("P0_EXT_PHY_ID : reg 17, value %x \n", regData);
			}
...
#ifdef CONFIG_RTL8367R_VB_API
static unsigned int rtk_get_port_stats(unsigned int port, rtk_stat_port_type_t cntr_idx)
{
        rtk_stat_counter_t counter;

        rtk_stat_port_get(port, cntr_idx, &counter);
 ...
 int rtl819x_get_port_stats(int portnum , struct port_statistics *port_stats)
 {
...
#ifdef CONFIG_RTL_8367R_SUPPORT
        if (portnum >= 0 && portnum <= 4)
#else // CONFIG_RTL_8370_SUPPORT
        if (portnum >= 0 && portnum <= 8) // 0~7 : LAN, 8 : WAN
#endif
        {
		//printk("RTL8367R PORT STAT GET:%d\n", portnum);
	...
	return 0;
        ...
        if (portnum >= 5 && portnum <= 8)
        {
                ...
                /* port_stats->rx_bytes = rtk_get_port_stats(iport, STAT_IfInOctets); */
                ...
                /* port_stats->tx_bytes = rtk_get_port_stats(iport, STAT_IfOutOctets); */
...
int re865x_priv_ioctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
	...
	if (cmd == RTL819X_IOCTL_L2_ADDR_GET)
	{
		if (copy_from_user(&mac.octet[0], (unsigned char *)rq->ifr_data, 6*sizeof(unsigned char)))
			return -EFAULT;
	}
	else
#endif
	...
	switch (cmd)
	{
	     ...
	     case RTL819X_IOCTL_READ_PORT_STATS:
		 	...
		 	break;
#if defined(CONFIG_RTL_8367R_SUPPORT)
	     case RTL819X_IOCTL_L2_ADDR_GET:
			L2_data.fid = 0;
 			L2_data.efid = 2; // LAN
			if (rtk_l2_addr_get(&mac, &L2_data) != 0)
                        {  
 			        L2_data.efid = 1; // WAN
			        if (rtk_l2_addr_get(&mac, &L2_data) != 0)
				     L2_data.port = 0xff;	
	 		...
	 		if (copy_to_user((void *)rq->ifr_data, (void *)&L2_data.port, sizeof(L2_data.port)))
				return -EFAULT;
			break;
#endif
	     default :
...
int read_config_header(saveconf_header_t *header)
{
        int ret;
        size_t rdlen;
        struct mtd_info *mtd;

        mtd = get_mtd_device_nm("boot+cfg+linux+rootfs");
        if (IS_ERR(mtd))
	{
		printk("READ MTD ERROR\n");
                return (int)mtd;
	}

		printk("===================> mtd->size : %llu\n", mtd->size);
        ret = mtd->read(mtd, mtd->size-0x10000, sizeof(saveconf_header_t), &rdlen, (u_char *)header);
        if (rdlen != sizeof(saveconf_header_t))
                printk("warning: read_config_header: rdlen is not equal to len\n");
...
//	rtlglue_printf("	creating eth0~eth%d...\n",totalVlans-1 );
	...
	read_config_header(&header);
	printk("header->MACIGC : %s\n", header.magic_str);
	printk("header->MACIGC : %08x\n", header.vlan_for_wan);
#endif

#ifdef  CONFIG_EFM_KT_3WAN_PORTS
        rtl_setIpv4UnknownMCastFloodMap(nicIgmpModuleIndex, 0x3);
        rtl_setIpv6UnknownMCastFloodMap(nicIgmpModuleIndex, 0x3);
#else
...
//#if defined(CONFIG_RTK_VLAN_FOR_CABLE_MODEM)
	...
	memset(&rx_skb_queue, 0, sizeof(struct ring_que));
        ...
        iptv_port_entry = create_proc_entry("iptv_port", 0, NULL);
        if (iptv_port_entry) {
                iptv_port_entry->write_proc = proc_write_iptv_port;
        }

        wan_port_type_entry = create_proc_entry("wan_port_type", 0, NULL);
        if (wan_port_type_entry) {
                wan_port_type_entry->write_proc = proc_write_wan_port_type;
        }
