...
int (*twinip_rx_handler)(struct sk_buff *skb) = NULL;
int (*twinip_tx_handler)(struct sk_buff *skb, char *ifname) = NULL;
#endif
...
int dev_queue_xmit(struct sk_buff *skb)
{
	...
	if (skb->ip_summed == CHECKSUM_PARTIAL) {
        ...
        if (twinip_tx_handler)
                twinip_tx_handler(skb, dev->name);
#endif
...
#if defined(CONFIG_DRIVERLEVEL_REAL_IPCLONE) || defined(CONFIG_DRIVERLEVEL_REAL_IPCLONE_MODULE)
        if ((twinip_rx_handler) && (twinip_rx_handler(skb) < 0))
        {
                printk("twinip_rx_handler....NET_RX_DROP\n");
                return NET_RX_DROP;
        }
#endif
