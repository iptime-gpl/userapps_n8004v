...
#if defined(CONFIG_RTK_VLAN_NEW_FEATURE) && defined(CONFIG_PORT_BASED_VLAN_FEATURE)
#error "Incompatible options: CONFIG_RTK_VLAN_NEW_FEATURE & CONFIG_PORT_BASED_VLAN_FEATURE both can't be enabled"
#endif
...
static inline int should_deliver(struct net_bridge_port *p, struct sk_buff *skb)
{
        ...
        else
        {
                ...
                /* For HW NAT product, all traffic from BR0 itself will be allowed */
                if(p->br && (!memcmp(eth_hdr(skb)->h_source,p->br->dev->dev_addr,6)))
                {
                        //printk("from br0:should_deliver -> %s:%d\n", skb->dev->name,skb->dev->br_port->port_no );
                        return 1;
                }
#endif
                return 0;
