	...
	if ((!(range->flags & IP_NAT_RANGE_PROTO_SPECIFIED) ||
			...
			((orig_tuple->dst.protonum==IPPROTO_TCP) ||(orig_tuple->dst.protonum==IPPROTO_UDP)))
		{
			...
			rtl865xNaptEntry.extPort = rtl865xNaptEntry.protocol?tuple->src.u.tcp.port:tuple->src.u.udp.port;
                        ...
                        // ysyoo, 2011-11-24, (sip == gip) -> start from a remote PC to the wan ip of this gateway.
                        if (rtl865xNaptEntry.intIp == rtl865xNaptEntry.extIp) goto out;
                        // The followings are only for connection which is made from a local PC.
#endif
